...
***秒杀请求来了，首先使用lua脚本确认秒杀资格并且处理缓存中的数据，主要是看当前优惠券库存是否足够以及用户是否到了领取上线，如果符合领取资格，先对缓存中的数据进行库存扣减并且增加领取次数。此时缓存已经变了，数据库中的信息还没更新，我们要更新这部分数据，因此使用事务来更新（update优惠券库存，insert领券信息），完成以上两步提交事务说明已经完成这部分的一致性处理。还有一些额外的操作异步去完成，比如更新用户的“我的优惠券”这部分的redis缓存，并且发送一个延时消息来修改用户领取的优惠券到期时间***
...
***此处还存在一个问题，如果lua脚本操作成功，但是数据库写入失败（宕机了），此时肯定会抛出异常，我们可以通过异步发送消息队列去回滚redis，或者，考虑到这种情况发生的概率很低，我们可以选择直接采用同步的方式回滚redis***
这段 SQL 确实是原子性的，并且它利用了 MySQL (InnoDB) 的**行级锁（Row-level Lock）**机制。
...
### Q7：Redis库存预扣减->数据库库存更新，数据库库存更新失败了怎么办
这是一个非常经典的数据一致性场景。

在 **“Redis 预扣减 (Lua) -> 数据库扣减 (DB)”** 的流程中，如果 **Redis 扣减成功**，但随后的 **数据库更新失败**（例如事务回滚、数据库宕机、网络超时等），结果就是：

- **Redis 库存**：少了 1 个（比如从 10 变 9）。
    
- **数据库库存**：没少（保持 10）。
    
- **用户端**：收到“失败”的响应（因为 DB 报错抛出异常了）。
    

这种情况被称为 **“少卖” (Under-selling)**。那个在 Redis 里被扣掉、但没在数据库里卖出去的库存，相当于“凭空消失”了（或者说被“冻结”了）。

针对这种情况，通常有三种处理方案，取决于你的业务对“少卖”的容忍度：
...
- **后果**：
    ...
    - Redis 里那个库存就永远丢了。如果 Redis 只有 10 个库存，数据库有 10 个。失败一次后，Redis 剩 9 个，数据库剩 10 个。最终 Redis 会先卖完（变成0），系统停止接单，而数据库里还剩 1 个卖不出去。
        
- **为什么这么做？**
    
    - **高性能**：不需要额外的网络开销去操作 Redis。
        
    - **安全性**：**超卖**（实际只有10个，卖了11个）会导致资损和法律风险，这是**绝对不行**的。但**少卖**（实际有10个，只卖了9个）对于优惠券这种虚拟资产，损失极小，且保证了系统兜底的安全。
        
    - 正如课程文档所言：“_只是一张优惠券而已，会有一些容忍故障。_”
...
**核心思想：在出错时，尝试把 Redis 的库存加回去。**

- 处理方式：
    
    在执行数据库操作的 try-catch 块中，如果捕获到异常，除了回滚数据库事务外，再发一个 Redis 命令把库存加回去。
    ...
    try {
        // 1. 扣减 DB 库存
        couponTemplateMapper.decrementStock(...);
    } catch (Exception e) {
        // 2. DB 失败，回滚事务
        status.setRollbackOnly();
    
        // 3. 【尝试回滚 Redis】
        // 发送一个 Lua 脚本或 INCR 命令，把 Redis 库存 +1
...
- **缺点**：
    ...
    - **不是 100% 可靠**：如果数据库报错的原因是因为**网络波动**，那么你执行第 3 步（回滚 Redis）的时候，可能网络还是不通的，导致 Redis 回滚也失败。
        
    - **增加延迟**：在异常处理流程中增加了网络 I/O。
...
**核心思想：定期以数据库为准，修正 Redis。**

这是大厂生产环境的标准做法，用来解决“少卖”问题。

- **处理方式**：
    
    1. 开发一个**定时任务**（比如每 5 分钟执行一次）。
        
    2. 任务逻辑：读取 MySQL 里的**真实剩余库存**。
        
    3. **强制覆盖**（`SET`）Redis 里的库存值。
        
- **效果**：
    
    - 即使发生了“Redis 扣了但 DB 没扣”的情况，造成了 Redis 库存偏少。
        
    - 5 分钟后，定时任务会发现差异，并把 Redis 的库存**修正**回来。
...
您提到的这两种方式，`version`（版本号）和 `stock > 0`（库存大于0），它们都是在高并发下保证数据一致性的手段，但它们解决的问题和思想**有本质的区别**。

`version` 是**真正的“乐观锁”**，而 `stock > 0` 是一种利用数据库原子性的**“业务约束型”**更新。

我们来深入对比一下。

#### 核心问题：什么是“丢失更新” (Lost Update)？

在对比之前，我们必须明确我们要解决的问题。在高并发下扣减库存，会遇到“丢失更新”问题：

1. **线程 A** 读到库存 `stock = 10`。
    
2. **线程 B** 读到库存 `stock = 10`。
    
3. **线程 A** 在应用中计算 `10 - 1 = 9`，执行 `UPDATE products SET stock = 9`。
    
4. **线程 B** 在应用中计算 `10 - 1 = 9`，执行 `UPDATE products SET stock = 9`。
    
5. **最终结果：** 卖出了 2 件商品，库存却只减少了 1。**数据出错了**。
    

`version` 和 `stock > 0` 都是为了解决这个问题，但思路完全不同。

---

#### 方案一：使用 `version` （标准乐观锁）

这是一种**“CAS (Compare-And-Swap)”**思想的实现。它假设冲突很少，所以不加锁，但在最后更新时“检查一下”数据是否被别人动过。

**实现方式：**

1. 在商品表 `products` 中增加一个 `version` 字段（通常是整型，默认为0）。
    
2. **第一步（读）：** `SELECT stock, version FROM products WHERE id = 1;`
    
    - （应用层拿到 `stock = 10`, `version = 5`）
        
3. **第二步（改）：** 在应用层计算新库存 `new_stock = 9`。
    
4. **第三步（写）：** `UPDATE products SET stock = 9, version = version + 1 WHERE id = 1 AND version = 5;`
    

**工作流程：**

- **线程 A** 执行 `UPDATE ... WHERE version = 5;`。成功，数据库中 `stock` 变为 9，`version` 变为 6。（影响行数 = 1）
    
- **线程 B** 几乎同时执行 `UPDATE ... WHERE version = 5;`。此时 `version` 已经是 6 了，`WHERE version = 5` 匹配不到数据。
    
- **线程 B** 的更新失败。（影响行数 = 0）
    
- **线程 B** 的应用层发现影响行数为 0，**得知“更新失败”**。它必须**重试**：重新执行第一步（读到 `stock=9`, `version=6`），然后再次尝试更新。
...
1. 通用性极强（最大的优点）：
    ...
    这是 version 方案的王牌。它不只适用于库存扣减。任何“先读后改”的业务（例如：编辑用户信息、修改订单状态、更新文章内容）都可以用它来保证数据一致性。
    
2. 严谨的数据一致性：
    
    它能完美解决“丢失更新”问题，并且能防止 ABA 问题（version 只会单向递增，不会变回去）。
    
3. 并发性能高：
    
    全程不使用数据库的行锁（Pessimistic Lock），线程之间不会阻塞，而是通过“失败重试”来保证一致性，吞吐量高。
...
1. 应用层必须实现“重试”逻辑：
    ...
    这是它最大的缺点。更新失败（影响0行）时，你不能简单地告诉用户“失败了”，而是需要应用层捕获这个“失败”，然后自动（或手动）重新拉取最新数据，再走一遍业务逻辑。这增加了代码的复杂度。
    
2. 数据库侵入性：
    
    需要在每一张需要乐观锁的表里都增加一个 version 字段。
    
3. 冲突粒度问题：
    
    只要是更新这一行数据，version 都会改变。比如，线程 A 更新了库存，线程 B 只是更新了这件商品的描述。线程 B 的更新也会因为 version 改变而失败，必须重试，即使它俩的业务本来并不冲突。
...
这个方案**严格来说并不是“乐观锁”**，它是一种更聪明的、利用数据库**原子操作**的技巧。

它**完全规避**了“先读后改”的周期，把“计算”这个步骤**直接交给了数据库**。
...
- **线程 A** 执行 `UPDATE ... SET stock = stock - 1 WHERE stock > 0;`。
    
    - 数据库（如 InnoDB）会自动为 `id = 1` 这一行**加锁**。
        
    - 检查 `stock`（此时为10）是否 `> 0`。是。
...
- **线程 B** 几乎同时执行。
    ...
    - 由于线程 A 持有锁，线程 B **阻塞等待**。
        
    - 线程 A 释放锁后，线程 B 拿到锁。
        
    - 检查 `stock`（此时为9）是否 `> 0`。是。
...
- **最终结果：** `stock` 变为 8。数据**完美一致**。
    
- **如果库存只剩 1：** 线程 A 执行成功，`stock` 变为 0。线程 B 再执行时，`WHERE stock > 0` 匹配失败，返回“影响行数 = 0”。
...
1. 极度简单（最大的优点）：
    ...
    应用层不需要“重试”逻辑（特指并发重试）。代码只有一行 SQL，非常清晰。
    
2. 性能极高：
    
    它避免了“SELECT + UPDATE”两次网络I/O，变成了一次I/O。数据库的原子性保证了它的安全。
    
3. 逻辑清晰：
    
    返回“影响行数 = 0”时，业务含义非常明确：“库存不足（卖完了）”，而不是“并发冲突”。
...
1. 适用场景极度局限（最大的缺点）：
    ...
    它只能用于这种“原子加减”的特定场景（如扣库存、加积分）。
    
    - **无法使用：** 如果你的业务是“先读取 N 个数据，在应用层进行复杂计算（比如风控、价格计算），最后把结果写回去”，这个方案**完全不适用**。
...
|**特性**|**version (乐观锁)**|**stock > 0 (原子更新)**|
|---|---|---|
|**核心思想**|先读后改，提交时检查（CAS）|直接在数据库内完成原子计算|
|**解决问题**|通用的“丢失更新”问题|特定的“库存扣减”问题|
|**通用性**|**极高**（适用于所有“读-改-写”流程）|**极低**（仅适用于原子加减）|
|**应用层复杂度**|**高**（必须实现失败重试逻辑）|**极低**（只需判断影响行数=0）|
|**性能**|高（但有2次I/O和重试开销）|**极高**（1次I/O，无并发重试）|
|**数据库侵入**|**是**（需要`version`字段）|**否**（使用已有业务字段）|

#### 结论：我该如何选择？

- 如果您的场景就是“扣库存”、“秒杀”：
    
    请毫不犹豫地使用 UPDATE ... SET stock = stock - 1 WHERE stock > 0;。这是该场景下最简单、最高效、最正确的实现。
    
- 如果您的场景是“编辑商品信息”：
    
    用户打开编辑页（读取数据），修改了价格、描述、库存（在应用层计算），然后点击“保存”（更新数据）。
    
    在这种场景下，stock > 0 完全无效。您必须使用 version 方案，来防止两个运营人员同时修改同一件商品导致数据覆盖。