...
#### 面试中问mvcc
MVCC 全称是 Multi-Version Concurrency Control，也就是多版本并发控制，是数据库用来解决并发访问问题的一种机制，主要目的是在保证数据一致性的同时，提高并发性能。 
简单说，它的核心思路是：当多个事务同时读写数据库时，不会直接对当前数据进行覆盖，而是为数据维护多个版本。每个事务看到的数据版本可能不同，这样就避免了传统锁机制中可能出现的频繁阻塞问题。 
拿 InnoDB 举例，它实现 MVCC 的关键是通过隐藏字段和 undo log。每行数据除了我们定义的字段外，还有几个隐藏字段，比如 DB_TRX_ID（记录最后修改该数据的事务 ID）、DB_ROLL_PTR（回滚指针，指向该数据的上一个版本，存在 undo log 里）。 当一个事务要修改数据时，InnoDB 不会直接修改原数据，而是先把旧数据复制到 undo log 里，形成一个历史版本，然后再修改当前数据，并更新 DB_TRX_ID 为当前事务 ID，DB_ROLL_PTR 指向 undo log 里的旧版本。这样，其他事务如果需要读取数据，根据自己的事务 ID 和隔离级别，就能找到对应的历史版本，而不会被当前修改事务阻塞。 
比如在可重复读隔离级别下，事务启动时会生成一个 Read View（读视图），里面记录了当前活跃的事务 ID 范围。读取数据时，会通过对比数据的 DB_TRX_ID 和 Read View，找到符合条件的版本（比如修改该数据的事务已提交，且是最新的可见版本），这样就能保证事务过程中看到的数据始终一致。 MVCC 的好处是读不加锁（快照读），读写不冲突，大大提升了并发效率，这也是 InnoDB 等存储引擎默认使用它的原因。
