...
public class SingletonStaticInner {
    ...
    // 2. 定义一个私有的静态内部类
    // 这个类不会随着 SingletonStaticInner 的加载而加载
    private static class SingletonHolder {
        // 只有在调用 getInstance() 时，才会触发 SingletonHolder 的加载
        // JVM 在类加载时保证了 INSTANCE 的创建是线程安全的
        private static final SingletonStaticInner INSTANCE = new SingletonStaticInner();
    }

    // 3. 公有获取方法
    public static SingletonStaticInner getInstance() {
        // 第一次调用此方法时，JVM 才会加载 SingletonHolder 并初始化 INSTANCE
...
1. **懒加载**：只有调用 `getInstance()` 时，内部类 `SingletonHolder` 才会被加载。
2. **线程安全**：类的静态变量初始化是在类加载时由 JVM 保证线程安全的。
3. **写法优雅**：比 DCL 简洁得多。
